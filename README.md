...
- 两个计分器：
  ...
  - **客户奖品**：每月按积分发放的奖品（SNG 门票、MTT 门票或自订种类），客人取走时即时扣减，完整记录存量和每一次异动
  - **客户名单**：一页看晒每位客户的资料、常规赛积分和名次、Sit and Go 成绩（全部／月度／年度）、现存奖品和未使用奖励，点开可看完整战绩和异动纪录
...
## 版面尺寸
即时排行榜有三套固定尺寸：手机（宽度 < 640px）、平板（640–1023px）、电视（≥ 1024px）。
字体大小不会随着滚动或画面高度改变。电视和平板放不下时会固定尺寸分页轮流显示（最多 3 页，每 10 秒翻页），手机直接滚动。
第一页显示前几名的大字版，之后的页面统一细行，可以放更多人。

## 微信小程序
API 同时支援 Cookie（网页）和 Bearer token（小程序）登入。
登入时在 body 加上 `tokenAuth: true`，回应会附带 token。
